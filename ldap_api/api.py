...
from ldap_service import convert_to_str, extract_value_from_array, company_from_ldap
...
class API:
    ...
    def get_person(person_id):
        ...
        ldap_response = ldap_service.__find_person(person_id)
        ...
        for option, group in ldap_service.GROUPS.items():
            result[option] = ldap_service.__get_group(ldap_response, group)

        if result.get('company'):
            company = ldap_service.__find_company_entry_by_name(result.get('company'))
            if company:
    ...
    def get_people(company_id, only_disabled=False):
        ...
        ldap_service = g.get('ldap_service', None)
        people = []
        company = ldap_service.__find_company(company_id)
        if company is None:
            return None
        ...
        for member_dn in ldap_response[0][1]['member']:
            ...
            if entry:
                ...
                if entry[1].get('active', [b'TRUE'])[0].decode('utf-8') == active:
                    for option, group in ldap_service.GROUPS.items():
                        person[option] = ldap_service.__get_group(entry, group)
    ...
    def get_company(company_id):
        ...
        """
        ldap_service = g.get('ldap_service', None)
        ldap_response = ldap_service.__find_company(company_id)
    ...
    def add_person(attributes):
        ...
        while create_attempts < LdapService.MAX_CREATE_ATTEMPTS:
            try:
                person_id = ldap_service.__next_id('uid')
                dn = 'uidNumber={},{}'.format(person_id, LdapService.LDAP_BASES['people'])

                ldap_service.__add_entry(dn, ldap_attributes)
                break
        ...
        for option, group in ldap_service.GROUPS.items():
            if group in add_group_flags:
                ldap_service.__add_user_to_group(dn, attributes['company'], group)

        ldap_service.__add_user_to_group(dn, attributes['company'], 'people')
    ...
    def add_company(attributes):
        ...
        while create_attempts < LdapService.MAX_CREATE_ATTEMPTS:
            try:
                company_id = ldap_service.__next_id('uniqueIdentifier')
    ...
    def modify_person(person_id, attributes):
        ldap_service = g.get('ldap_service', None)
        person = ldap_service.__find_person(person_id)
        if person is None:
            return None
            ...
            if option in attributes:
                ...
                if company_name:
                    if attributes[option]:
                        attributes.pop(option)
                        ldap_service.__add_user_to_group(person[0], company_name, group)
                    else:
                        ldap_service.__remove_user_from_group(person[0], company_name, group)
                    # if company name changes remove from old company and add it to new company
                    if company_name != old_attributes['company'][0]:
                        ldap_service.__add_user_to_group(person[0], company_name, 'people')
                        ldap_service.__remove_user_from_group(person[0], old_attributes['company'][0], 'people')

        ldap_service.__modify_ldap_entry(person, attributes)
    ...
    def modify_company(company_id, attributes):
        ...
        ldap_filter = ldap.filter.filter_format('(o=%s)', [company_name])
        ...
        if staff:
            for person in staff:
                ...
                ldap_service.__modify_ldap_entry(person, person_attributes)

        ldap_service.__modify_ldap_entry(company, attributes)
    ...
    def delete_company(company_id):
        ldap_service = g.get('ldap_service', None)
        dn = 'uniqueIdentifier={},{}'.format(company_id, LdapService.LDAP_BASES['companies'])
        notify = ldap_service.__find_group(dn, 'notify')
        if notify:
            ldap_service.ldap_connection.delete_s(notify[0])

        approvers = ldap_service.__find_group(dn, 'approvers')
        if approvers:
    ...
    def search(name, base, get_disabled=False):
        ...
        if base == 'companies':
            ...
            ldap_filter = ldap.filter.filter_format('|(cn~=%s)(cn=%s*)(sn=%s*)(mail=%s)', [name, name, name, name])
