...
class BaseModel {
  ...
  static post(that, url, data, callback) {
    axios.post(url, JSON.stringify(data), { baseURL: Config.baseUrl() })
      .then(function(response) {
        if(response.data.status === 'success')
          callback(that, response);
        else {
  ...
  static put(that, url, data, callback) {
    axios.put(url, JSON.stringify(data), { baseURL: Config.baseUrl() })
      .then(function(response) {
        if(response.data.status === 'success')
          callback(that, response);
        else {
  ...
  static patch(that, url, data, callback) {
    axios.patch(url, JSON.stringify(data), { baseURL: Config.baseUrl() })
      .then(function(response) {
        if(response.data.status === 'success')
          callback(that, response);
        else {
